...
const getUserPlaylists = asyncHandler(async (req, res) => {
  ...
  const image = await Video.findById(getPlaylist[0]?.videos[0]);
  return res
    .status(200)
    .json(new ApiResponse(200, {getPlaylist, image: image?.thumbnail || null}, "Playlist fetched successfully"));
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(200, addVideo, "Video added successfully");
});
...
const updatePlaylist = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, updatedPlatlist, "Playlist updated successfully"),
    );
